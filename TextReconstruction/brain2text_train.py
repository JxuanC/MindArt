...
from data.dataset_augment import get_visual_text_fmri_dataset
from data.configure import CLIPGPTFEATURE, RETRIEVAL_MODEL, RETRIEVAL_INDEX
...
def get_model_and_auxiliaries(args):
    ...
    dataset, dataloader, fmri_dim = get_visual_text_fmri_dataset(args.dataset, args.sub, selected_rois, args.batch_size, 
                                                                    tokenizer = tokenizer, mixup = True, candidate = True,
                                                                    clip_features = CLIP_FEATURES, retrieval_k = args.k if not args.disable_rag else 0, 
                                                                    retrieval_model_path = args.retrieval_model_path if not args.disable_rag else None, 
                                                                    retrieval_index_path = args.retrieval_index_path if not args.disable_rag else None,
                                                                    template_path = args.template_path)

    encoder_config = fMRIViTEncoderConfig(fmri_dim, len(selected_rois), 768, ENCODERPARAMS[args.encoder_cog], 
                                          ENCODERPARAMS[args.encoder_cog], use_fmrigm = True, 
                                          fmri2img = False, fmrigm_path = args.retrieval_model_path)
    ...
    for param in model.encoder.fmrigm.parameters():
...
if __name__ == '__main__':
    ...
    parser.add_argument("--template_path", type=str, default="./data/template_retrieval.txt", help="TXT file with template")
    parser.add_argument("--n_epochs", type=int, default=60, help="Number of training epochs")
    ...
    parser.add_argument("--sub", type=str, default='sub-3')
    parser.add_argument("--dataset", type=str, default='DIR')
    parser.add_argument("--ROI", type=str, default='VC')
    parser.add_argument("--retrieval_model_path", type=str, default=f'{RETRIEVAL_MODEL}')
    parser.add_argument("--retrieval_index_path", type=str, default=f'{RETRIEVAL_INDEX}')
